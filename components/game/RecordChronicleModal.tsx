...
export const RecordChronicleModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const [activeTab, setActiveTab] = useState<'records' | 'treasury'>('records');
    ...
    // Helper to get chronicle content
    const getChronicleContent = (id: string) => {
    ...
    const readingItem = readingChronicleId ? SHOP_ITEMS.find(i => i.id === readingChronicleId) : null;
    const readingContent = readingChronicleId ? getChronicleContent(readingChronicleId) : "";
    ...
    // Filter unlocked rewards to hide chronicles from the "Collection" view
    const visibleUnlockedRewards = profile.unlockedRewards.filter(id => {
    ...
    const renderTreasury = () => (
        <div className="flex flex-col h-full overflow-y-auto pr-2 animate-in zoom-in-95 duration-300">
            {/* Header / Stats */}
            <div className="text-center mb-8 bg-[#2c1810]/5 p-6 rounded-lg border border-[#2c1810]/20 relative">
                <div className="flex justify-center mb-4">
                     <div className="w-24 h-24 rounded-full border-4 border-double border-[#8a1c1c] bg-[#2c1810] flex items-center justify-center text-[#f3e5ab] text-4xl font-fantasy font-bold shadow-xl">
                        {currentLevel}
                     </div>
                </div>
                <h3 className="text-2xl font-fantasy font-bold text-[#2c1810] mb-1">{t.messengerLevel}</h3>
                <div className="text-sm opacity-60 font-bold mb-4">{profile.rankPoints} / {nextLevelXp} {t.rankPoints}</div>
                
                <div className="inline-flex items-center gap-2 bg-[#f3e5ab] border border-[#2c1810] px-4 py-2 rounded-full shadow-inner">
                    <Icons.Coin className="w-6 h-6 text-[#2c1810]" />
                    <span className="text-2xl font-fantasy font-bold text-[#8a1c1c]">{profile.guildMarks}</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white/40 rounded-lg border-2 border-[#2c1810]/20 p-4 shadow-inner">
                    ...
                    <div className="space-y-2">
                        {/* Always show the Empty Guild Chronicle */}
                         <button 
                         ...
                         {Object.keys(profile.inventory).length === 0 ? (
                             <p className="italic opacity-50 text-center py-4 text-xs">{t.emptyInventory}</p>
                         ) : (
                             Object.entries(profile.inventory).map(([id, value]) => {
                                 ...
                                 return (
                                     <button 
                                        key={id} 
                                        disabled={true} // Consumables aren't usable here yet
                ...
                <div className="bg-white/40 rounded-lg border-2 border-[#2c1810]/20 p-4 shadow-inner">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        >
                            <span className="text-xs font-bold leading-tight">Default Theme</span>
                            {!profile.activeThemeId && <span className="text-[10px] text-green-700 font-bold mt-1">Active</span>}
                        </button>

                        {visibleUnlockedRewards.map(id => {
                            const item = SHOP_ITEMS.find(i => i.id === id);
                            if (!item) return null;
                            const IconComponent = (Icons as any)[item.iconKey] || Icons.Default;
                            ...
                            return (
                                <button 
                                    ...
                                    className={`flex flex-col items-center p-3 border rounded text-center transition-all ${
                                        isActiveTheme 
                                ...
                                >
                                    ...
                                    {isActiveTheme && <span className="text-[10px] text-green-700 font-bold mt-1">Active</span>}
                                    {item.type === 'theme' && !isActiveTheme && <span className="text-[10px] text-[#2c1810]/50 font-bold mt-1 underline">Equip</span>}
                                </button>
                            );
                        })}
                        {visibleUnlockedRewards.length === 0 && (
                             <div className="col-span-2 text-center py-4 italic opacity-50 text-xs">
                                 - - -
                             </div>
                        )}
                    </div>
                    ...
                    <div className="absolute inset-0 z-[55] p-8 bg-[#f3e5ab] animate-in slide-in-from-left duration-300 overflow-y-auto">
                        <div className="max-w-3xl mx-auto flex flex-col h-full">
                            <div className="flex justify-between items-center border-b-2 border-[#2c1810] pb-4 mb-6">
                                <h2 className="text-4xl font-fantasy font-bold text-[#2c1810]">{t.chronicleBookTitle}</h2>
                            ...
                            <h3 className="text-center font-bold font-serif italic mb-8 opacity-70 text-xl">{t.tableOfContents}</h3>
                                ...
                                {SHOP_ITEMS.filter(item => item.type === 'chronicle').map((item, index) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            onClick={() => {
                                                if (isUnlocked) {
                                                    setReadingChronicleId(item.id);
                                                }
                                            }}
                                            disabled={!isUnlocked}
                                            className={`w-full text-left p-6 border-b border-[#2c1810]/20 flex justify-between items-center group transition-colors ${
                                                isUnlocked ? 'hover:bg-[#2c1810]/5' : 'opacity-60 cursor-not-allowed bg-black/5'
                                            }`}
                                        >
                                            <div className="flex items-center gap-4">
                                                <span className="font-fantasy font-bold text-2xl w-10 text-right opacity-50">{index + 1}.</span>
                                                <div>
                    ...
                    <div className="absolute inset-0 z-[60] p-8 bg-[#f3e5ab] animate-in slide-in-from-bottom duration-500 overflow-y-auto">
                        <div className="max-w-2xl mx-auto space-y-6">
                            <div className="flex justify-between items-center border-b-2 border-[#2c1810] pb-2">
                                <h2 className="text-3xl font-fantasy font-bold text-[#8a1c1c]">{readingItem?.name[settings.language]}</h2>
